  ...
  return (
    <>
      <style>{`
        ...
        .gh-btn:hover { color: #000; border-color: #be0000; }
      ...
      <section id="github" ref={sectionRef} className="gh-section">
        ...
        {/* Web corner */}
        <svg className="web-corner" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
          {[40, 80, 120, 160, 200, 240].map((r, i) => (
            <circle key={i} cx="300" cy="0" r={r} fill="none" stroke="white" strokeWidth="0.8" />
          ))}
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          ...
          {/* ===== STAT CARDS ROW ===== */}
          <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 fade-up ${visible ? "show" : ""}`}
            style={{ transitionDelay: "0.15s" }}>
            {[
              { label: "Repositories", value: ghData?.public_repos ?? "—", sub: "public projects" },
              { label: "Followers", value: ghData?.followers ?? "—", sub: "web-heads" },
              { label: "Following", value: ghData?.following ?? "—", sub: "in the network" },
              { label: "Contributions", value: totalContribs || "—", sub: "last 365 days" },
            ].map((s, i) => (
              <div key={s.label} className="stat-card"
                ...
                <p className="stat-value">
                  {loading ? (
                    ...
                    <>{String(s.value).slice(0, -1)}<span>{String(s.value).slice(-1)}</span></>
                  )}
                </p>
          ...
          {/* ===== CONTRIBUTION GRAPH ===== */}
          <div className={`mb-8 fade-up ${visible ? "show" : ""}`}
            ...
            <div className="contrib-wrapper">
              ...
              <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span style={{ fontFamily: "'Share Tech Mono', monospace" }}
                    className="text-[10px] text-zinc-600">Less</span>
                  {[0, 1, 2, 3, 4].map((lvl) => (
                    <div key={lvl} className={`legend-cell ${getLevelColor(lvl)}`} />
                  ))}
                  <span style={{ fontFamily: "'Share Tech Mono', monospace" }}
                    className="text-[10px] text-zinc-600">More</span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex flex-col gap-[3px] pt-5 shrink-0">
                  {DAYS.map((day, i) => (
                    <div key={day}
                      style={{ fontFamily: "'Share Tech Mono', monospace", height: "12px" }}
                      className={`text-[9px] text-zinc-700 leading-[12px] ${i % 2 === 0 ? "opacity-100" : "opacity-0"}`}>
                      {day}
                    </div>
                  ))}
                </div>
                ...
                <div className="contrib-scroll flex-1">
                  <div className="contrib-inner">
                        ...
                        weeks.map((_, wi) => {
                          ...
                          return (
                            <div key={wi} style={{ width: "12px", flexShrink: 0 }}>
                              {ml && (
                                <span style={{ fontFamily: "'Share Tech Mono', monospace" }}
                                  className="text-[9px] text-zinc-500 whitespace-nowrap block leading-none">
                                  {ml.label}
                                </span>
                              )}
                            </div>
                    ...
                    {loading ? (
                        ...
                        {Array.from({ length: 53 }).map((_, wi) => (
                          <div key={wi} className="flex flex-col gap-[3px]">
                            {Array.from({ length: 7 }).map((_, di) => (
                              <div key={di} className="skeleton" style={{ width: "12px", height: "12px", borderRadius: "2px" }} />
                            ))}
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex gap-[3px] ">
                        {weeks.map((week, wi) => (
                          <div key={wi} className="flex flex-col gap-[3px]  w-full">
              ...
              <div className="mt-5 pt-4 border-t border-white/5 flex flex-wrap gap-6">
                {[
                  ...
                  <div key={item.label}>
                    <p style={{ fontFamily: "'Share Tech Mono', monospace" }}
                      className="text-[9px] text-zinc-600 tracking-widest mb-1">{item.label}</p>
                    <p style={{ fontFamily: "'Share Tech Mono', monospace" }}
                      className="text-xs text-red-400">{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ===== PROFILE + CTA ROW ===== */}
          <div className={`flex flex-col md:flex-row gap-6 items-stretch fade-up ${visible ? "show" : ""}`}
            ...
            <div className="profile-card flex-1">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <p style={{ fontFamily: "'Bebas Neue', sans-serif", letterSpacing: "0.05em" }}
            ...
            {/* CTA + tagline */}
            <div className="flex flex-col justify-between gap-6 shrink-0 md:min-w-[260px]">
              <div
                style={{ fontFamily: "'Share Tech Mono', monospace" }}
                className="bg-zinc-950/60 border border-white/5 border-l-2 border-l-red-700 p-5"
              >
                <p className="text-[10px] text-zinc-600 tracking-widest mb-2">// WEB_FINGERPRINT</p>
                <p className="text-sm text-zinc-300 leading-relaxed">
                  Every commit is a web strand.<br />
